...
import paxcomLogo from "./../assets/Company Logos/Paxcom Logo.png";
import mindfireLogo from "./../assets/Company Logos/Mindfire Logo.png";
import sapeintLogo from "./../assets/Company Logos/Sapient Logo.png";
...
const App_Experience = ({ activeRoute, handleMenuChange }) => {
  ...
  const [activeTab, setActiveTab] = useState(tabs.filter(tab=>window.location.pathname?.includes(tab.url))[0]?.title);
  
  let tabsView = tabs.map((tab, index) => (
    <div className={"company-tab col "+(tab.title===activeTab ? " active":'')} key={tab.title}>
  ...
  useEffect(() => {    
    if (window.location.pathname === App_Routes.EXPERIENCE.ROOT) {
      setActiveTab(tabs.filter(tab=>tab.url===App_Routes.EXPERIENCE.default)[0].title);
